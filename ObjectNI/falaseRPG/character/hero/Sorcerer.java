...
/**
 * 魔法使いのクラス 必殺技：ファイアのメソッド
 * 
 *
 */
public class Sorcerer{
	public String name = "次郎";
	public int hp = 100;
	public int maxHp = 100;
	public int attack = 15;
	public boolean escaped = false;
	public String job = "魔法使い";
	...
	public void randomWakeUp() {
		...
		int randomWakeUp = random.nextInt(3);
		if (randomWakeUp > 0) {
			System.out.println(name + "は目覚めた");
		} else {
			System.out.println(name + "は目覚めない");
		}
	}
	...
	public boolean special(MatangoParty targets) {
		Random random = new Random();
		System.out.println(name + "のファイア，敵全員が炎の渦に巻き込まれた");
		for (Matango target : targets.matangos) {
	...
	public boolean special(BigMatango target) {
		Random random = new Random();
		System.out.println(name + "のファイア，敵全員が炎の渦に巻き込まれた");
		if (!target.isDead() && !target.isEscaped()) {
			int randomAttack = random.nextInt(attack) + attack;
	...
	public boolean special(KingDemon target) {
		Random random = new Random();
		System.out.println(name + "のファイア，敵全員が炎の渦に巻き込まれた");
		if (!target.isDead() && !target.isEscaped()) {
			int randomAttack = random.nextInt(attack) + attack;
	...
	public boolean special(Dragon target) {
		Random random = new Random();
		System.out.println(name + "のファイア，敵全員が炎の渦に巻き込まれた");
		if (!target.isDead() && !target.isEscaped()) {
			int randomAttack = random.nextInt(attack) + attack;
