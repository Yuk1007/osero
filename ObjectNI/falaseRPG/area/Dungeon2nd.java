...
public class Dungeon2nd{	
		...
		while (true) {
			...
			if (nextMove == 1) {
				...
				if (result.equals("WIN")) {
					Dungeon3rd dungeon3 = new Dungeon3rd();
					dungeon3.access(party);
	...
	/*	Random random = new Random();
		...
		Demon[] m = new Demon[numOfMonster];
	...
	private String encountBoss(HeroParty party) {
		...
		KingDemon kd = this.nextBoss();
		String result = BattleField.battle(party, kd);
		this.printBattleResult(result, kd.name);
		return result;
	}
	...
	protected KingDemon nextBoss() {
		return new KingDemon();
	}

	
	protected Dungeon3rd nextArea() {
